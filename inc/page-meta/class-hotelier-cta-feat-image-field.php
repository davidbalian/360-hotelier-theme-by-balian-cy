...
final class Hotelier_Cta_Feat_Image_Field {
	...
	/**
	 * When the bottom CTA field has no stored meta on a managed page, return the attachment
	 * matching the schema's `cta_feat_img.default_url` for that page's context.
	 *
	...
	public static function load_image_value( $value, $post_id, $field ) {
		...
		if ( ! is_array( $fields ) || ! isset( $fields['cta_feat_img']['default_url'] ) ) {
			return $value;
		}

		$url = trim( (string) $fields['cta_feat_img']['default_url'] );
		if ( '' === $url ) {
			return $value;
		}
	...
	/**
	 * Map a post ID to a CTA schema context based on front-page status / page template.
	 * Returns '' when the post is not in a CTA-managed location.
	 */
